<template>
  ...
  >
    ...
    <div
      ...
      <template v-if="markList2.length > 0">
        <div>
          <div
            v-for="(item, key) in markList2"
            :style="getStopStyle(item.position)"
            class="el-slider__stop2 el-slider__marks-stop2"
            :key="key">
          </div>
        </div>
        <div class="el-slider__marks2">
          <slider-marker
            :mark="item.mark" v-for="(item, key) in markList2"
            :key="key"
            :style="getStopStyle(item.position)">
          </slider-marker>
        </div>
  ...
  export default {
    ...
    computed: {
      ...
      markList2() {
        ...
        const marksKeys = Object.keys(this.marks2);
        return marksKeys.map(parseFloat)
          .sort((a, b) => a - b)
          .filter(point => point <= this.max && point >= this.min)
          .map(point => ({
